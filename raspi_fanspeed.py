...
parser.add_argument('-C', '--cmd', type=str, help='execute command and pipe temperature and speed', default='/usr/bin/mosquitto_pub -r -t \'home/{hostname}/RPi.fanspeed/json\' -m \'{message}\''.format(hostname=hostname, message='{message}'))
parser.add_argument('-t', '--temp', type=float, help='temperature preset', default=25.0)
parser.add_argument('-v', '--verbose', action='store_true', default=False)
parser.add_argument('--log', type=str, help='Write temperature and speed into this file instead of executing --cmd', default=None) #'/var/log/tempmon.json'
